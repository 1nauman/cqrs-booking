...
*   **Valkey (Redis) & StackExchange.Redis:** Employed for a distributed **Seat Locking Service**. This uses atomic `SET NX EX` commands to prevent multiple users from concurrently reserving the same seat, with built-in expiry for resilience.
...
This `GEMINI.md` provides a comprehensive overview of the `cqrs-booking` project, its architecture, technologies, and how to get started.
